...
export class BookComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && BookComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (BookComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IBook[] | null): IBook[] {
    const booksNew = this.books ?? [];
    if (data) {
      for (const d of data) {
        if (booksNew.map(op => op.id).indexOf(d.id) === -1) {
          booksNew.push(d);
        }
      }
    }
