...
public class BookRepositoryWithBagRelationshipsImpl implements BookRepositoryWithBagRelationships {
    ...
    public Optional<Book> fetchBagRelationships(Optional<Book> book) {
        return book.map(this::fetchRecipes);
    }
    ...
    public List<Book> fetchBagRelationships(List<Book> books) {
        return Optional.of(books).map(this::fetchRecipes).orElse(Collections.emptyList());
    }

    Book fetchRecipes(Book result) {
        return entityManager
            .createQuery("select book from Book book left join fetch book.recipes where book is :book", Book.class)
            .setParameter("book", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Book> fetchRecipes(List<Book> books) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, books.size()).forEach(index -> order.put(books.get(index).getId(), index));
        List<Book> result = entityManager
            .createQuery("select distinct book from Book book left join fetch book.recipes where book in :books", Book.class)
