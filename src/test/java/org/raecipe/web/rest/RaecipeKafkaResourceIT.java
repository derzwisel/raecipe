...
import org.raecipe.config.KafkaProperties;
...
class RaecipeKafkaResourceIT {
    ...
    void setup() {
        ...
        RaecipeKafkaResource kafkaResource = new RaecipeKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/raecipe-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/raecipe-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
