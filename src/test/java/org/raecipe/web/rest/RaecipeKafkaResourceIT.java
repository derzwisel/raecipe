...
import org.raecipe.config.EmbeddedKafka;
import org.raecipe.config.KafkaSseConsumer;
import org.raecipe.config.KafkaSseProducer;
...
class RaecipeKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/raecipe-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/raecipe-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/raecipe-kafka/unregister"));
                return;
            }
        }
